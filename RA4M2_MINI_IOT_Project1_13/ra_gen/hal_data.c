...
{ .scan_mask = ADC_MASK_CHANNEL_11 | 0,
  .scan_mask_group_b = 0,
...
/* Actual period: 0.000125 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x2bf2,
  .duty_cycle_counts = 0x15f9, .source_div = (timer_source_div_t) 0, .channel = 6, .p_callback = gpt6_callback,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.1 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x895440,
  .duty_cycle_counts = 0x44aa20, .source_div = (timer_source_div_t) 0, .channel = 0, .p_callback = gpt0_callback,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
