...
def load_cloud_config() -> CloudConfig | None:
    ...
    if not token:
        missing.append("ALIYUN_NLS_TOKEN")
    if not agentrun_url:
...
def main() -> int:
    ...
    cloud_config = None if args.no_cloud else load_cloud_config()
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
            ...
            if args.no_cloud:
                print("Cloud pipeline: disabled by --no-cloud")
            elif cloud_config is None:
                print("Cloud pipeline: disabled because configuration is missing")
            else:
                print("Cloud pipeline: enabled")
            ...
            while True:
                ...
                with conn:
                    ...
                    try:
                        ...
                        print(f"Audio receive error: {exc}", file=sys.stderr)
                        if args.once:
                            break
                        continue
                ...
                if cloud_config is not None:
                    try:
                        ...
                        print(f"\nCloud pipeline error: {exc}", file=sys.stderr)
                else:
                    print("Audio received successfully. Cloud processing was skipped.")
