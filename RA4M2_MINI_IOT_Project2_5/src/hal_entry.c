...
static const char* g_label = "idle";
...
void hal_entry(void)
{
  ...
  //  platform_init(dev_ctx.handle);
    ...
    while (1)
    {
        ...
        //这里每圈查询一次 DRDY（data ready）标志。
        //drdy_xl 为真代表 有新的加速度样本（在 60 Hz 下约每 16.7 ms 置位一次）。
        if (drdy.drdy_xl) {
            ...
            //把 mg 改成 g（/1000）。Python 训练脚本也按 g 读入，这样两端一致。
           float ax = (float)acceleration_mg[0] / 1000.0f;
           float ay = (float)acceleration_mg[1] / 1000.0f;
           float az = (float)acceleration_mg[2] / 1000.0f;

            // 只在采集中输出 CSV，单位转成 g，便于后续直接训练
            //采集中才输出 CSV，并在满帧数后自动结束
            if (g_capturing) {

                //t_ms 用的是 HAL_GetTick() 相对会话起点的差值，便于回放；精度在毫秒级，足够。
                uint32_t t_ms = hal_millis() - g_start_ms;
               // CSV: t_ms,ax,ay,az,label
               printf("%lu\t%.6f\t%.6f\t%.6f\t%s\t\r\n", (unsigned long)t_ms, ax, ay, az, g_label);
                       //按我们设定：60 Hz × 10 s → CAPTURE_SAMPLES = 600。收满 600 行就自动结束会话。
               if (++g_count >= CAPTURE_SAMPLES) {
                   g_capturing = 0;
                   printf("---END---\r\n"); // 结束标记，可选
               }
            }
